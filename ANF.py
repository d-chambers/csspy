...
Read directory of unziped csspy files into pandas data frame
"""
import glob
import os
...
def readANF(anfdir, lon1=-180, lon2=180, lat1=0, lat2=90, getPhases=False, UTC1='1960-01-01',
    ...
    """Function to read the csspy directories as downloaded from the csspy Earthscope Website"""
    ...
    for month in monthDirs:
        ...
        DF = DF[(DF.Lat > lat1) & (DF.Lat < lat2) & (DF.Lon > lon1) &
                (DF.Lon < lon2) & (DF.time > utc1) & (DF.time < utc2)]

        if getPhases:
            dfAssoc = _readAssoc(glob.glob(os.path.join(month, '*.assoc'))[0])
            dfArrival = _readArrival(
                glob.glob(os.path.join(month, '*.arrival'))[0])
...
def readOrigerr(origerrFile):
    columnNames = ['orid', 'sobs', 'smajax',
                   'sminax', 'strike', 'sdepth', 'conf']
    columnSpecs = [(0, 8), (169, 179), (179, 188), (189, 198),
                   (199, 205), (206, 215), (225, 230)]
    df = pd.read_fwf(origerrFile, colspecs=columnSpecs,
...
def _readOrigin(originFile):
    ...
    columnSpecs = [(0, 9), (10, 20), (20, 29), (30, 47), (48, 56), (57, 65), (66, 74), (75, 79), (80, 84),
                   (85, 89), (90, 98), (99, 107), (108, 110), (111,
                                                               115), (116, 125), (126, 128), (128, 136),
                   (136, 144), (145, 152), (153, 161), (162,
                                                        169), (170, 178), (179, 194), (195, 210),
                   (211, 219), (220, 237)]

    df = pd.read_fwf(originFile, colspecs=columnSpecs,
                     header=False, names=columnNames)
    df['DateString'] = [obspy.core.UTCDateTime(
        x).formatIRISWebService() for x in df.time]
    return df
...
def _readAssoc(assocFile):
    ...
    df = pd.read_fwf(assocFile, colspecs=columnSpecs,
                     header=False, names=columnNames)
    return df


def _readArrival(arrivalFile):
    columnNames = ['sta', 'time', 'arid',
                   'stassid', 'iphase', 'amp', 'per', 'snr']
    columnSpecs = [(0, 6), (7, 24), (25, 33), (43, 51),
                   (70, 78), (136, 146), (147, 154), (168, 178)]
    df = pd.read_fwf(arrivalFile, colspecs=columnSpecs,
                     header=False, names=columnNames)
    return df
...
def _linkPhases(DF, dfAssoc, dfArrival, Pcodes, Scodes):
    ...
    for a in DF.iterrows():
        # DF associated with orid, should be one row
        dfas = dfAssoc[dfAssoc.orid == a[1].orid]
...
def ANFtoTemplateKey(anfDF, temKeyName='TemplateKey_anf.csv', saveTempKey=True):
    ...
    contrib = ['csspy'] * len(anfDF)
    mtype = ['ML'] * len(anfDF)
    stakey = ['StationKey.csv'] * len(anfDF)
    df = pd.DataFrame()
    df['CONTRIBUTOR'], df['NAME'], df['TIME'], df['LAT'], df['LON'], df['DEPTH'], df['MTYPE'], df['MAG'], df[
        'STATIONKEY'] = contrib, ds, ts, anfDF.Lat.tolist(), anfDF.Lon.tolist(), anfDF.depth.tolist(), mtype, anfDF.ml.tolist(), stakey
    if saveTempKey:
...
def makePickTimes(ANF, stakey):
    ...
    for num, row in ANF.iterrows():
        ...
        for sta in stainter:
            Path = os.path.join('EventWaveForms', 'evename',
                                'TA.' + sta + '.' + evename + '.pkl')
            Ptime = row.Picks[sta][0]
            Stime = row.Picks[sta][1]
            starttime = Ptime
            endtime = Stime if Stime > Ptime + 30 else Ptime + 30
            df = pd.DataFrame([[evename, sta, Ptime, Stime, Path, starttime, endtime]], columns=[
                              'Name', 'Station', 'P', 'S', 'Path', 'Startttime', 'Endtime'])
            DFlist.append(df)
